...
int combustivel = 400;
int pontos = 0;
int enemySpawn, Ohp, muniT, flagO=0;
...
int gameOver() {
    ...
    if ( (tabuleiro[(altura/2) - posicaoJogador][0] == '+') && (tabuleiro[(altura/2) - posicaoJogador][1]=='<') ) {
        /*Caso de colisao com tiro inimigo*/
        return 3;
    }
        if (combustivel <=0) {
...
void destroiAlvo(enemyO O) {
    ...
    for (i=0; i<altura; i++) {
        ...
        for (j=largura; j>=0; j--) {
            ...
            if ( (tabuleiro[i][j] == '>') && (tabuleiro[i][j+1] == 'O') ) {
                O.hp--;
...
void inimigoParaEsquerda(int i, int j, enemyO O) {
    ...
    if (tabuleiro[i][j] == 'X') {
    ...
    else if (tabuleiro[i][j] ==  O.img) {
        tabuleiro[i][j - 1] = O.img;
        tabuleiro[i][j] = ' ';
    }
        ...
        if ( (tabuleiro[i][j] == '<') && ( (tabuleiro[i][j-1] == 'X') || (tabuleiro[i][j-1] == 'O') || (tabuleiro[i][j-1] == 'F') || (tabuleiro[i][j-1] == 'T')  ) ) {
...
void jogoMapa(enemyO O) {
    ...
    for (i=0; i<altura; i++) {
        ...
        for (j=0; j<largura; j++) {
            ...
            if ( (tabuleiro[i][j] == 'X') || (tabuleiro[i][j] == O.img) || (tabuleiro[i][j] == 'T') || (tabuleiro[i][j] == '<') ) {
            
                /*Tem inimigo? Move pra esquerda*/
                destroiAlvo(O);
                inimigoParaEsquerda(i, j, O);
            }
        ...
        for (j=0; j<largura; j++) {
            ...
            if (i==0 || i==altura-1) {
                ...
                if (spawn <= enemySpawn ) {                        
                    ...
                    else if ( (objeto2 <= probO) && (flagO == 0) ) {
                        
                        /*Spawn O*/

                        flagO++;    /*flag que so permite o spawn de um O*/

                        O.img = 'O';
                        O.hp = Ohp;

                        tabuleiro[ posicaoO][j] = O.img;
                    }
...
int main () {
    ...
    if (fimJogo != 0) {
        ...
        if (fimJogo == 1) {
            ...
            /*Texto printado caso Game Over por colisao com tiro inimigo*/
            printf("                                    Ora bolas, voce nao desviou do tiro inimigo!\n");
            printf("\n");
            printf("                                    Sua pontuacao foi: %d \n", pontos);
            printf("\n                                    Pressione qualquer tecla para sair do jogo.\n");  
        }
